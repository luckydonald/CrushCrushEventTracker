...
## Summary
Prevent only Codex implementation-start prompts from being appended verbatim to `ai/°base/query.md` when they contain the forwarded-plan boilerplate and repeat the plan that was just saved. Leave Claude prompt logging behavior unchanged.

## Key Changes
- In `scripts/°base/ai/hooks/save-prompt/hook.py`, run the new normalization only when `ai_tool == "codex"`.
...
- If the Codex prompt contains that latest plan text after the boilerplate, strip the boilerplate plus exactly that plan block and preserve only any user-authored text after it.
...
- Add coverage for a Codex forwarded prompt followed by a real user instruction after the matched saved plan; assert `query.md` contains only that instruction, prefixed with `›`.
- Add coverage proving Claude logs the same text unchanged.
- Add coverage where the newest plan file is tiny and matching is rejected.
