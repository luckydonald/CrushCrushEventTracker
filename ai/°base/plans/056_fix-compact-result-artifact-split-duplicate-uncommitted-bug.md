...
Confirmed scope for Claude **and** Codex: `.claude/settings.json` / `.codex/hooks.json` wire `PreCompact`, `PostCompact` (`matcher: "manual|auto"`), and `SessionStart` to the same hook scripts symmetrically for both tools — the fix must not assume Claude-only payload shape. `custom_instructions()` already checks 4 differently-named keys for this reason; keep that. Codex's observed `SessionStart` payload and transcript do **not** expose Claude's plaintext compact-summary schema; it must fail soft for artifact capture, but still record the compaction in `query.md`.

"Non-prompt" usage = `trigger == "auto"` compaction (no user-typed instructions) and the legacy `SessionStart(source=compact)` fallback. The current `PreCompact` hard-drop before `custom_instructions()` (`save-compact-prompt/hook.py:63-65`) explains the missing event record; replace it with minimal-block handling that skips only the absent instruction text. `PostCompact` already handles both triggers. The fix must degrade cleanly to "no quoted instructions line" when there's nothing to quote.
...
- [x] Codex automatic compact (`trigger: auto`) — observed in this session; payload details below.
...
Conclusion: a Codex bare compact has no hook-visible plaintext result with the current event/transcript contract. Do **not** add a decoder or rely on the encrypted transcript blob. The implementation must use `session_id` as the stable directory correlation fallback when an artifact is available, but otherwise write a minimal `query.md` compaction entry with no artifact links; this records the event without creating a misleading empty artifact. This changes the prior plan's Codex assumption and requires dedicated no-summary and session-id fallback tests.

### Findings — Codex automatic compact (this repo, session_id `019fdb75-51c0-7ef2-8b85-94b2fdd31a14`)

- `20260807-153302_465766-save-compact-prompt.json` — `PreCompact`, `trigger: auto`, with `session_id`, `turn_id`, and `transcript_path`, but no `prompt_id`, instruction, or summary.
- `20260807-153403_097443-save-compact-prompt.json` — `PostCompact`, same session/turn and `trigger: auto`, but no `compact_summary`; `capture_postcompact()` returns `False`.
- `20260807-153403_202016-record-memory.json` — `SessionStart`, `source: compact`, same session and transcript path, but no summary-bearing field. The transcript has no Claude `compact_boundary` / `isCompactSummary` records. Its post-compact context exposes only the literal `"auto"`, not the compressed conversation, so `compact_summary_from_transcript()` returns `None`.

Conclusion: this confirms the no-plaintext limitation for the automatic Codex path too. The hook must reserve/reuse the `session_id` directory and append a minimal automatic compaction entry even though it cannot create a `result.md`; the entry is an auditable event record, not an empty-result claim.
...
Wrap each compact-artifact operation in a repository-local advisory lock: reserve directory and metadata → write artifact → render/upsert query block → stage → commit. `_commit_paths` executes inside that lock, so retry handles transient git failures but cannot allow two writers to interleave their query edits. Use a private temporary Git index initialized from `HEAD`, so this commit contains only the compact transaction and never mutates the user's real index. Snapshot the real index and working-tree overlays for `query.md` before committing; after the successful compact commit, automatically rebase and restore both overlays onto the new `HEAD`. For the compact directory being upserted, the hook-owned generated block wins; user edits outside it are preserved. Never abort a hook awaiting manual replay because restoration is imperfect: commit the compact transaction, restore deterministically, and leave diagnostics only for unexpected preservation cases. Route `append_and_commit` and the three ad hoc commit sites (`save-compact-prompt/hook.py`, `save-prompt/hook.py`, `record-memory/hook.py`) through the centralized primitive, removing duplicated unchecked `subprocess.run` pairs.
...
- Every `PreCompact`, including `trigger: auto`, reserves the directory and stores the canonical trigger in compact metadata. Manual `/compact <text>` stores that text in the same metadata for later inline rendering. It also upserts and commits a minimal `query.md` block immediately, even if no artifact can ever be captured; later writers replace that same block with its artifact links. Later writers read the stored trigger; SessionStart must never infer or overwrite it.
- `capture_session_start` writes `result.md` only when the summary belongs to the newest compact boundary; if that boundary's summary is not yet available, it no-ops rather than selecting an earlier boundary.
- A lone direct PostCompact summary is also named `result.md`, so one-artifact compactions keep the simple, familiar name. Metadata records whether that `result.md` came from the direct payload or transcript reconstruction. If a valid transcript result subsequently arrives while `result.md` is direct, the locked transaction promotes the existing file to `analysis.md`, writes the transcript as the new `result.md`, and upserts the block in one commit. If transcript arrives first, PostCompact writes `analysis.md` directly. Byte-identical sources stay as one `result.md`.
...
### 3. Render the `/compact` instruction text directly in `query.md`, drop `output/compacted/`

- `save-compact-prompt/hook.py`'s `PreCompact`-manual branch stops writing `output/compacted/NNN.md` / the "possible prompt" query.md line. Instead: add `compact_result.ensure_prompt_directory(log_path, payload) -> Path` (allocates/reuses the `NNN.<uuid>` directory for this `prompt_id`, or `NNN.<session_id>` for Codex, same numbering as today) and persist the supplied `/compact <text>` suffix in compact metadata.
- `PreCompact` renders and commits a minimal block immediately. It includes the canonical `output/compact/NNN.<correlation-id>/` path as its stable visible identity and the optional quoted manual suffix, but has no artifact links yet. This is required for a Codex event that has no plaintext result. When the first result artifact is available, `render_compact_block()` replaces the same block and adds its links; no `instructions.md` is materialized.
- Non-`manual` triggers still have nothing to quote, but they no longer early-return before the event-record block is written.

### 4. Regenerate one deterministic block per compaction directory instead of appending fragments

- Add `render_compact_block(directory: Path, trigger: str) -> str`: reads the pending manual instruction from compact metadata (if present) for the quoted line, then emits links for `analysis.md` as `Analysis (…)`, `result.md` as `Result (…)`, and `autoloads.md` in that fixed order. This fixes both the "two blocks" bug and the "arrival-order-dependent line ordering" issue while making the two different summary sources clear.
...
- Add `upsert_query_block(log_path, directory, block_text, *, extra_commit_paths, commit_template_relpath, default_commit_msg) -> bool` in `_lib.py`. Locate an existing compact block from its unique visible `output/compact/NNN.<correlation-id>/` directory path (present in the minimal heading as well as artifact links), then replace that complete compact block; append when no such directory path exists. Do not add HTML comments or any other invisible Markdown markers. Commit `query.md` + every changed artifact/metadata file in one locked `_commit_paths` call.
...
- Update `test_precompact_manual_with_instructions_writes_file` → expects pending instructions in compact metadata, no `output/compacted/` path, and no immediate `query.md` change.
...
- New promotion/recovery tests: direct-first creates only `result.md`; a later distinct valid transcript promotes direct `result.md` to `analysis.md` and stores the transcript in `result.md` in one commit; an injected failure during that transition leaves no broken committed query link and retries from metadata/temp files safely.
- New Codex tests: (a) observed bare and automatic Codex PreCompact/PostCompact/SessionStart payloads with no plaintext summary produce no artifact but exactly one minimal query entry, and (b) a synthetic Codex payload with a usable summary but no `prompt_id` reuses its `session_id` directory and replaces that entry with links. Do not assert that Codex's encrypted transcript can be reconstructed.
