...
- Every `PreCompact`, including `trigger: auto`, reserves the directory and stores the canonical trigger in compact metadata. Manual `/compact <text>` stores that text in the same metadata for later inline rendering. No query block is written at this stage. Later writers read the stored trigger; SessionStart must never infer or overwrite it.
...
- `save-compact-prompt/hook.py`'s `PreCompact`-manual branch stops writing `output/compacted/NNN.md` / the "possible prompt" query.md line. Instead: add `compact_result.ensure_prompt_directory(log_path, payload) -> Path` (allocates/reuses the `NNN.<uuid>` directory for this `prompt_id`, same numbering as today, no result artifact written yet) and persist the supplied `/compact <text>` suffix in compact metadata. No `query.md` write occurs yet.
- When the first result artifact is available, `render_compact_block()` reads the pending instruction from metadata and emits it inline as the blockquoted line below `❯ Conversation compacted …`; it is never materialized as `instructions.md`.
...
- Add `upsert_query_block(log_path, directory, block_text, *, extra_commit_paths, commit_template_relpath, default_commit_msg) -> bool` in `_lib.py`. Locate an existing compact block from its unique `output/compact/NNN.<correlation-id>/` directory path already present in one of that block's artifact links, then replace that complete compact block; append when no such directory path exists. Do not add HTML comments or any other invisible Markdown markers. Commit `query.md` + every changed artifact/metadata file in one locked `_commit_paths` call.
